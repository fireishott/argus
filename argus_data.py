...
def _provider_credential(provider: str) -> str:
    """Read only the service's already-staged runtime environment value.

    Background Keychain access can trigger a macOS SecurityAgent prompt that
    blocks the desktop. The local service must never call Keychain.
    """
